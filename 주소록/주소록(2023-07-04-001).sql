...
-- 주소록 + 취미정보가 있는 데이터를 정규화하여 Relation 설정하기
...
제 1 정규화가 된 취미 데이터는 취미1, 취미2, 취미 3 칼럼이 만들어진 table 이다.
이 테이블에 만약 4번째 취미를 추가하고 싶으면, 
    새로운 칼럼을 추가하거나 
    4번째 취미를 다른 취미 항목으로 교체해야 한다.
    
    만약 취미 항목이 늘어나 칼럼을 추가하게 되면 
    TABLE 구로를 변경해야 하고, 상당히 많은 비용이 요구된다.(시간 등)
    또한 Table 구조를 변경해야 하는 것은 매우 신중하게 처리해야 한다.
...
DDL 명령 : DBA 가 사용하는 명령들, 물리적 객체 생성, 제거, 변경을 수행하는 명령
    CREATE : 객체 생성, 최초 프로젝트를 시작할 때
    DROP : 객체 제거, 최초 프로젝트 시작할 때 잘못 생성된 객체를 다시 만들고자 할때
...
제 2 정규화가 되어 있는 tbl_addr_hobby TABLE 에는
주소록의 ID(hb_aid) 와 취미 이름(hb_name)이 저장되어 있다.
...
WHERE hb_name = '원래 이름'
과 같은 SQL 을 사용해야 한다.
이 SQL 은 데이터를 다수 변경하는 명령이 없다.
UPDATE 명령은 기본적으로 PK를 기준으로 1개의 데이터만 변경되도록 하는 것이 좋다.
하지만 이 멸영은 이러한 원칙에 위배된다.
...
1. 기존의 Table 에서 일반 문자열로 되어있는 부분을 떼어 새로운 Table 에 옮긴다.
2. 기존 Table 과 새로운 Table 간에 Relation 설정을 한다.
*/

-- 1. tbl_hobby table 의 이름 변경 : Entity 를 Relation 으로 이름변경
ALTER TABLE tbl_hobby 
RENAME TO tbl_addr_hobby;

SELECT * FROM tbl_addr_hobby;

-- 2. 취미 Relation 에서 취미 이름 항목을 중복 데이터 없이 추출하기
--      중복 없이 데이터를 모으기
...
-- 3. 제3정규화 데이터 import
-- 주의! 기존 제 2정규화 데이터를 엑셀로 보내서 제 3정규화 데이터가 완료된 후
-- 다음을 진행할 것
...
-- Table JOIN
/* 
정규화(2,3정규화) 된 Table 정보를 Select (조회)하면
단순히 1개의 Table 만으로는 어떤 정보를 보여주고 있는지 알 수가 없다.
2,3 정규화 된 Table 은 필연적으로 Table 간의 JOIN 이 되어야만 정보를
명확히 할 수 있다.
JOIN 은 2개 이상의 Table 을 서로 연계하여 필요하느 정보를 확인하는 것이다.
*/
...
-- tbl_addr_hobby Relation 과 tbl_address Entity 테이블을 나열하되
-- 두 테이블의 칼럼 ah_aid 와 a_id 의 데이터가 일치한 정보만 나열하라

-- EQ JOIN
-- EQ JOIN 은 모든 JOIN 방식중에 가장 빠르다.
-- 단 참조무결성 제약 조건이 완벽해야 한다.
-- EQ JOIN 은 연계하는 TABLE 간에 참조 무결성 제약조건이 완벽할때는 
-- 아무런 문제가 없이 결과를 조회할 수 있다.
-- 하지만 연계하는 Table 간에 참조 무결성 제약조건이 위배되면
-- 조회된 데이터를 신뢰할 수 없다.
SELECT * FROM tbl_addr_hobby, tbl_address
WHERE ah_aid = a_id;
...
-- LEFT JOIN
-- JOIN 을 실행하는 Table 간에 참조무결성이 의심스럽거나 위배될지 모른다는 전제하에
-- 데이터를 확인하는 JOIN
-- FROM[table1] LEFT JOIN [table2]
-- 왼쪽(LEFT)의 table1 데이터는 무조건 SELECT 하고
-- 오른쪽 table2 데이터는 ON 에서 설정한 조건에 맞는 것만 가져오기
-- 오른쪽 Table2의 조건에 맞는 데이터가 있으면 보여주고
--      없으면 null 로 표현
-- 1. SELECT 한 데이트의 누락이 발생하지 않는다.
-- 2. 참조 무결성 제약 조건이 완벽하게 성립하는지 확인하는 용도.

SELECT ah_aid, a_name, a_tel, a_addr, ah_hbcode 
FROM tbl_addr_hobby LEFT JOIN tbl_address ON ah_aid = a_id
ORDER BY ah_aid;

-- 주소 취미 Relation 과 주소 Entity 간의 참조 무결성 검사
...
-- 연계된 Entity, Relation 간에 참조관계가 잘 유지되도록, Table 에 제약조건을 설정하는 것
-- 1. FK 는 Relation Table 에 설정한다.
-- 2. FK 는 1:N의 관계 table 에서 N 의 Table 에 설정한다.
