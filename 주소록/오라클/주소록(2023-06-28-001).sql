...
-- tbl_address TABLE 에서 A_ID 값의 최대값, 최소값 확인하기
...
-- SELECT 명령에서 모든 칼럼(*) 으로 조회하는 것 보다는 ALL Projection 으로
-- 조회를 하는 것이, 조회 속도 면에서 더 유리하다.
-- 코딩(java .. 등등)과 연동하여 사용할 때는 가급적 ALL Projection 을 사용하는 것이 좋다
SELECT a_id, a_name, a_tel, a_addr
FROM tbl_address;


-- ID 가 A0010 이하인 데이터 들만 보여달라
...
3. WHERE 절의 조건대로 SELECTION
4. SELECT 절의 칼럼대로 데이터 PROJECTION
5. ORDER BY 절의 칼럼을 기준으로 정렬

SELECT FROM 은 필수항목 WHERE ORDER BY 는 선택항목
*/
...
-- 많은 양의 데이터 중에서 SELECTION 을 수행할 때 
...
-- 전화번호 6 글자가 090-33 으로 시작되는 데이터 SELECTION
SELECT * FROM tbl_address
WHERE a_tel LIKE '090-33%';


-- id 값이 A0010 부터 A0020 인 데이터만 Selection 하라
SELECT *
FROM tbl_address
...
/*
SQL 에서 WHERE 등 조건을 부여하여 SELECTION 을 할 때 
특히 다중 조건을 부여할 때 ,
조건문의 순서에 따라 SELECT 결과의 효율이 달라질 수 있다.
조금더 깊이 생각하여 효율적인 SQL 명령문을 고민해 보자

1. 첫번째 SELECTION
    먼저 범위를 제한하여 개수를 제한하고 전화번호를 찾는다.
2. 두번째 SELECTION
    전체 범위에서 전화번호를 찾고 그중에서 범위를 제한하여 개수를 제한한다.
*/
