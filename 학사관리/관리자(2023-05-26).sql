...
--      가장 먼저 생성해야할 객체
-- User : DBMS 서버에 로그인 하고, 자신이 관리할 데이터들과 연결하는 객체
--          오라클에서 USER는 데이터 저장소의 개념이다. 이 개념은 다른 DBMS 와는 약간 다르게 취급한다.
--          저장소 SCHEME 라고 표현한다.
...
-- student TABLESPACE를 생성하고 저장소는 STUDENT.DBF로 하라. 
-- 초기 용량을 1MByte로 설정하고 1KByte 씩 자동 증가하도록 설정하라
CREATE TABLESPACE student -- 저장소의 이름을 student 라는 이름으로 사용하겠다
DATAFILE 'C:/app/data/student.dbf' -- 저장소 폴더에 STUDENT.DBF 라는 이름으로 생성
SIZE 1M AUTOEXTEND ON NEXT 1K; -- 초기에 저장소 공간을 1M(byte) 확보하고
...
-- 만약 student 라는 사용자를 생성하고 싶으면 C## 라는 이름으로 생성을 해야한다.
-- 이러한 정책은 보안적인 명에서 권장하지만 때로는 불편할때가 있다.
