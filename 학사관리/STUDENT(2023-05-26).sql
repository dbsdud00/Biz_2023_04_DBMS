...
-- DBA 권한은 테이블 생성, 수정 삭제, 데이터의 CRUD, COMMIT ROLLBACK 등의 명령을 
--  수행 할 수 있다.

-- 사용자 입장에서 데이터 저장소 생성
-- 실제 논리적인 데이터를 관라하는 객체
-- TABLE
...
*/
-- 데이터를 추가하기 전에 Table 생성 오류가 발견되면 제거하고 다시 생성할 수 있다.
-- DROP TABLE tbl_student; -- Table 제거하기
CREATE TABLE tbl_student (
    st_num	VARCHAR2(6)		PRIMARY KEY,
...
-- 테이블의 구조를 변경하는 명령은 비용이 매우 크기때문에
-- 사용할 때 항상 주의를 해야 한다.
-- 특히 칼럼의 크기(길이)를 변경하거나, type 을 변경할 때는
...
INSERT INTO tbl_student(st_num, st_name, st_dept, st_grade, st_tel)
VALUES ('230002', '성춘향', '법학과', 2, '010-333-2211');


INSERT INTO tbl_student(st_num, st_name, st_dept, st_grade, st_tel)
VALUES ('230003', '홍길동', '행정학과', 1, '010-222-4411');
...
-- 조건 부여 검색, 조건 부여 SELECT, 조건 부여 조회
SELECT st_num, st_name, st_dept, st_grade, st_tel
FROM tbl_student
WHERE st_name = '홍길동';
...
-- 학생 이름으로 정렬하고 같은 이름이 있으면 
-- 그 범위내에서 전화번호로 한번 더 정렬하여 보여라
SELECT st_num, st_name, st_dept, st_grade, st_tel
FROM tbl_student
...
-- 결과적으로 심각한 문제를 안고 있다.
-- 홍길동이라는 이름이 동명이인이 있을 경우, 원하지 않는 데이터가 변경될 수 있다.
...
-- 이몽룡(전화번호가 010-333-1111) 학생이 자퇴를 하고 이민을 갔다.
