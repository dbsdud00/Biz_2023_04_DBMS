...
tbl_buyer TABLE에 저장된 데이터를 아무 조건 없이 모두 조회하여 보여달라.
 >> SELECT * FROM tbl_buyer;
*/
SELECT * FROM tbl_buyer;

-- tbl_buyer TABLE 에 저장된 데이터를 아무 조건없이 보이되
-- buid, buname 칼럼(field, 속성) 만 보여달라
-- 저장된 데이터 중에서 id 와 이름 항목만 보고싶다.
SELECT buid, buname FROM tbl_buyer;

-- 이름과 전화번호 칼럼만 리스트로 보여라.
SELECT buname, butel FROM tbl_buyer;

-- tbl_buyer TABLE에 저장된 데이터 중에서 buname(이름) 항목의 데이터가 
-- 성춘향 으로 되어있는 데이터들만 보고싶다.
-- WHERE 절 : 조건을 부여할 떄
SELECT * FROM tbl_buyer
WHERE buname = '성춘향';
...
-- 이름이 홍길동으로 되어있는 데이터 리스트를 보고싶다.
-- 데이터가 없다 이런경우 결과가 Empty인 상태이다.
SELECT * FROM tbl_buyer
WHERE buname = '홍길동';

/*
buname 칼럼을 기준으로 조건 설정 하였다.
buname 칼럼에 어떤 값을 조건으로 데이터를 조회한 것
buname 칼럼은 데이터가 추가될 때 같은 이름의 데이터가 중복하여 저장될 수 있다.
buname 칼럼을 기준으로 조회한 데이터가 비록 현재는 1개만 보이더라도 
    이 데이터는 0개 이상의 데이터가 조회될 것이다
    라는 것을 반드시 예상해야 한다.

만약 buname 칼럼을 기준으로 하여 데이터를 Update, Delete 를 수행할 경우
다수의(2개이상) 데이터에 변경(수정)이 이루어 질 수 있다.
그러한 이유로 Update, Delete를 수행할때는 
    절대 buname 칼럼을 기준으로 삼아서는 안된다.
*/
...
/*
buid 칼럼을 기준으로 조회하기
buid 칼럼을 TABLE을 기준으로 create 할 때 PK(PRIMARY KEY) 성질을 설정하였다.
PK 로 설정된 칼럼은 유일성, NOT NULL 속성을 갖게 된다.

만약 이 칼럼에 이미 있는 데이터를 또다시 추가하려고 하면 
                                                다음과 같은 오류가 밣생한다.
...
데이터를 추가할 때 (INSERT)할 때 최소한 buid 값과 buname 값을 필수로
필요하다 라는 선언
...
DBMS 에서는 개발자, 사용자가 바라보는 모든 데이터는 TABLE(표) 형식이다.

TABLE을 작성하기 앞서, 개념적, 논리적 모델링을 수행하는데 
    이 단계에서는 TABLE을 Entity라고 한다.

프로그래밍(Java 등) 에서는 TABLE 을 잭체, 등으로 취급한다.
...
-- 매우 위험한 코드
-- UPDATE 나 DELETE를 수행할 때는 PK 가 아닌 칼럼을 기준으로 하지 마라.
-- 만약 PK 가 아닌 칼럼을 기준으로 할 때는 매우 신중하게 명령을 수행하야 한다.
...
3. buid(PK 칼럼)을 기준으로 Update를 실시한다.
*/
...
-- 데이터 추가
-- 컬럼 목록 개수, 순서 = 데이터 목록 개수, 순서 일치해야 한다.
INSERT INTO [table] (컬럼목록)
VALUE(데이터 목록);
...
INSERT INTO tbl_acc(acnum, acdiv, acbuid, acbalance)
values('2023052302','1','0003',50000);

INSERT INTO tbl_acc(acnum, acdiv, acbuid, acbalance)
values('2023052303','1','0003',10000);

SELECT * FROM tbl_acc;
/*
현재 계좌정보를 조회했는데 
고객정보가 고객 ID 뿐이어서 고객에 대한 이름, 전화번호 등을 알 수가 없다.
고객 정보와 계좌정보를 연계하여 함께 볼수 있다면 좋겠네

2개의 Table 을 서로 연계하여 하나의 리스트로 보기
*/
...
-- Projection : SELECT 조회를 할 때 별표(*)를 사용하지 않고 컬럼을 나열하는 것
...
-- SQL Developer 와 Java 코드에스 DB를 서로 연동하여 처리하는 경우 발생하는 문제
-- SQLD 에서 데이터를 INSERT, UPDATE, DELETE 를 수행하는 경우
--      추가, 수정, 삭제된 정보는 아직 storage 에 반영되지 않고, 메모리에 임시 보관된 상태이다.
-- 이 상태일 때 Java 에서 SELECT 를 수행하면 INSERT, UPDATE, DELETE 된 데이터가아닌 이전 상태의 데이터가 조회된다.
-- 간혹 이 상황에서 DBMS 가 Connection 에서 무한정 응답하지 않는 경우도 있다.
-- java는 DBMS 가 응답하기를 기다리면서 무한정 기다리고 마치 프로젝트가 멈춘(Down)상태가 되어버린다.

-- SQLD 에서 INSERT, UPDATE, DELETE 를 수행한 다음에는 강제로 storage에 Commit을 해주어야 한다.
