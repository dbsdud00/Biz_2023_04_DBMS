...
SELECT acnum,acdiv,acbuid,acbalance FROM tbl_acc;

SELECT * FROM tbl_acc;

SELECT *
From tbl_acc
WHERE acnum = '2023052302';

INSERT INTO tbl_acc(acNum, acdiv, acbuid, acbalance)
VALUES ('2023052402','1','0003',10000);
...
-- 고객 ID 가 0001 인 고객에게 계좌정보를 잔액순으로 정렬하여 보이기
-- 오름차순(기본값)(ASENDING). 생략 가능
SELECT * FROM tbl_acc
WHERE acbuid = '0001'
ORDER BY acBalance ASC;

-- 고객 ID 가 0001 인 고객에게 계좌정보를 잔액이 큰것부터 순서대로 정렬하여 보이기
-- 내림차순(DESENDING)
SELECT * FROM tbl_acc
WHERE acbuid = '0001'
ORDER BY acBalance DESC;

-- 고객 ID 0001과 0003인 고객의 계좌정보 조회
SELECT * FROM tbl_acc
...
-- acBuId 에 0001 도 아니고 0003 도 아닌 값의 조건 검색
SELECT * FROM tbl_acc
...
-- 구조 변경을 할 때 칼럼의 크기는 절대 줄여서 변경하면 안된다.

-- tbl_acc 의 acNum 칼럼의 길이가 VARCHAR(10) 인데 
...
-- max(칼럼) : 전체 리스트(조건에 맞는 데이터 중에서) 중에서 가장 큰 값을 찾기
-- min(칼럼) : 전체 리스트(조건에 맞는 데이터 중에서) 중에서 가장 작은 값 찾기
